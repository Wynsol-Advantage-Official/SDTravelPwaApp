...
export function ItineraryTimeline({ days, accommodations = [], destination }: ItineraryTimelineProps) {
    ...
    return (
      <section className="mt-12">
        ...
        <div className="rounded-2xl border border-white/20 bg-white/10 p-8 text-center shadow-lg backdrop-blur-md">
          <MapPin className="mx-auto mb-3 h-8 w-8 text-gold/60" />
          <p className="font-serif text-lg font-semibold text-charcoal/80">
  ...
  return (
    <section className="mt-12">
      <h2 className="mb-8 font-serif text-2xl font-bold text-charcoal">
        Your Journey, Day by Day
      </h2>
        ...
        {days.map((day, index) => {
          ...
          return (
            <li key={day._id || `day-${day.dayNumber}`}>
              ...
              {index !== 0 && <hr className="bg-gold/30" />}

              {/* ── Day badge (timeline-start) ─────────────────────────── */}
              <div className="timeline-start timeline-box border-0 bg-transparent p-0 text-right">
                <span className="inline-flex h-9 w-9 items-center justify-center rounded-full border-2 border-gold bg-diamond font-serif text-sm font-bold text-charcoal shadow-sm">
                  {day.dayNumber}
                </span>
              </div>
              ...
              <div className="timeline-middle">
                <div className="h-3 w-3 rounded-full bg-gold shadow-[0_0_8px_rgba(212,175,55,0.4)]" />
              </div>
                    ...
                    if (hasAcc) {
                      return (
                        <div className="grid h-48 gap-0.5 md:grid-cols-3">
                          <div className="relative col-span-3 md:col-span-2 overflow-hidden bg-sand/5">
                            {accImg ? (
                              <Image
                                src={accImg.src}
                              ...
                              <div className="flex h-full items-center justify-center">
                                <MapPin className="h-8 w-8 text-sand/40" />
                              </div>
                            )}
                          </div>
                  ...
                  <div className="p-5">
                    ...
                    {(day.meals || day.accommodation) && (
                      <div className="mt-4 flex flex-wrap gap-2">
                        {day.meals && (
                          <span className="inline-flex items-center gap-1.5 rounded-full border border-gold/20 bg-gold/5 px-3 py-1 text-xs font-medium text-charcoal/75">
                        ...
                        {day.accommodation && (
                          (() => {
                              ...
                              return (
                                ...
                                >
                                  <div className="overflow-hidden rounded-xl border border-ocean/15 bg-white/30 shadow-sm backdrop-blur-sm transition-shadow hover:shadow-md">
                                    ...
                                    {/* Hero image */}
                                    <div className="relative h-36 w-full overflow-hidden bg-sand/10">
                                      {img ? (
                                        <Image
                                          src={img.src}
                                      ...
                                      {matched.rating > 0 && (
                                        <span className="absolute right-2 top-2 flex items-center gap-1 rounded-full bg-white/90 px-2 py-0.5 text-xs font-semibold text-charcoal backdrop-blur-sm">
                                      ...
                                      {matched.amenities?.length > 0 && (
                                        <div className="mt-2 flex flex-wrap gap-1">
                                          {matched.amenities.slice(0, 3).map((a) => (
                                            <span
                                              key={a}
                                              className="rounded-full border border-sand/30 bg-sand/10 px-2 py-0.5 text-xs text-charcoal/60"
                                            >
                                              {a}
                                            </span>
                                          ))}
                            ...
                            return (
                              <span className="inline-flex items-center gap-1.5 rounded-full border border-ocean/20 bg-ocean/5 px-3 py-1 text-xs font-medium text-charcoal/75">
              ...
              {!isLast && <hr className="bg-gold/30" />}
            </li>
          )
        })}
      </ul>
